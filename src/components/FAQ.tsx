...
const faqData: FAQItem[] = [
  ...
  },
  {
    ...
    answer: 'Sim. A proposta é justamente ajudar você a organizar o tempo disponível e definir prioridades, independentemente de quando começou a estudar.',
  },
  {
    id: 4,
    question: 'Qual a diferença entre as duas opções?',
    answer: 'O Sprint ENEM por R$12,99 dá acesso à plataforma de organização e estudos. No pacote de R$22,98 você recebe o Sprint ENEM e também o Sprint Redação, um material complementar para ajudar no planejamento, escrita e revisão da redação.',
  },
];
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full px-5 sm:px-6 py-4 sm:py-5 flex items-center justify-between text-left gap-4 hover:bg-slate-100/60 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 cursor-pointer"
                >
                  ...
                  <div
                    className={`w-7 h-7 rounded-full bg-white border border-slate-200 flex items-center justify-center shrink-0 transition-transform duration-200 ${
                      isOpen ? 'rotate-180 bg-purple-50 border-purple-200 text-purple-600' : 'text-slate-500'
                    }`}
                  >
                ...
                {isOpen && (
                  <div className="px-5 sm:px-6 pb-5 pt-1 text-sm text-slate-600 leading-relaxed border-t border-slate-200/80 animate-fadeIn">
                    <p>{item.answer}</p>
                  </div>
                )}
              </div>
